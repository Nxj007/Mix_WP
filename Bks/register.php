...
<body>
  ...
  $nameErr = $emailErr = $lnameErr = $genderErr = $ageErr = $hobErr = $studyErr = $imgErr = $passErr = $cpassErr = "";
  ...
  <div class="container mb-3">
    ...
    <form id='form' name="myphp" method="post" onsubmit="return validateForm()" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" enctype="multipart/form-data" required>
      ...
      <div class="mb-3 ">
        <div class="form-group">
          ...
          <input type="name" class="form-control" placeholder="Last Name" name="emp_lname" required>
      ...
      <div class="mb-3">
        ...
        <div class="form-group">
          ...
          ?>
              <input type="checkbox" id=study name=studylist[] value="<?php echo $study['s_id']; ?>"><?= $study['emp_study']; ?>
    ...
    if (isset($_POST["inset-btn"])) {
      ...
      //php validation
      $duplicate = mysqli_query($conn, "select `username` from emp_data where `username`='$username' ");
      if (mysqli_num_rows($duplicate) > 0) {
       ...
       try {
            ...
            $insert = "INSERT INTO emp_data(username,password,emp_fname,emp_lname,emp_age,emp_img,emp_details,emp_gender,user_type)VALUES('$username','$pass','$employee_fname','$employee_lname','$employee_Age','$image','$employee_details','$checkgen','$employee_user')";
            ...
            foreach ($study as $item1) {
              ...
              $run_insert2 = mysqli_query($conn, $data1);
            }
...
<script>
  ...
  function validateForm() {
    ...
    // }
    // if (newvar == 1) {
    //   document.getElementById('studyErr').innerHTML = "please select"
    //   return false;
    // }
    // var form_data = new FormData(document.querySelector("form"));
    // if(!form_data.getAll("studylist[]"))
    //     {
    //       printError("studyErr", "Please select");

    //       }
    // if(!form.studylist.checked) {
    //   printError("studyErr","The checkbox IS NOT checked");
    // } else {
    //     printError("studyErr", "");
    //     studyErr = false;
    //   }
    ...
    var nameErr = lnameErr = emailErr = passErr = cpassErr = ageErr = hobErr = studyErr = genErr = true;
